...
public class AgendaNewDialog extends javax.swing.JDialog {
     ...
     * @param eventService
     */
    public AgendaNewDialog(java.awt.Frame parent, boolean modal, AppointmentService eventService, DonorService donorService) {
        super(parent, modal);
        initComponents();
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_jButton2ActionPerformed
        ...
        if (selectedDate != null) {
            LocalDateTime localDateTime = selectedDate.toInstant()
                    ...
                    .toLocalDateTime();
            event.setDate(localDateTime); // Usa o LocalDateTime
        }
        ...
        for (int row = 0; row < tableContacts.getRowCount(); row++) {
            Object phoneValue = tableContacts.getValueAt(row, 0); // Supondo que a coluna de telefone seja a terceira
            // (índice 2)
            if (phoneValue != null) {
                if (phones.length() > 0) {
        ...
        event.setCall(call);
        event.setCall(call);
        event.setEventType("Ligação");
        event.setNotes(txtObs.getText());

        event.setDonation(donation);

        var id = this.service.save(Application.loggedUser(), event);
        if (id != null) {
    ...
    private void updateContactsTable(Donor donor) {
        ...
        if (donor.getContacts() != null) {
            ...
            for (DonorContact contact : donor.getContacts()) { // Supondo que o método getPhones() retorna uma lista de
                // Strings
    ...
    private void updateDonationTable(Donation donation) {
        ...
        // Limpa a tabela
        model.setRowCount(0);
        model.addRow(new Object[]{donation.getAmount(), donation.getReceivedAt()});
